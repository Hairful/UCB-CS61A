...
def balanced(m):
    ...
    return (torque_l == torque_r) and balanced(end(left(m))) and balanced(end(right(m)))
...
def totals_tree(m):
    ...
    if is_planet(m):
        return tree(mass(m))
    branches = [total_weight(end(f(m))) for f in [right, left]]
...
def replace_loki_at_leaf(t, lokis_replacement):
    ...
    new_branch = [replace_loki_at_leaf(b, lokis_replacement) for b in branches(t)]
